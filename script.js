document.addEventListener('DOMContentLoaded', () => {
  ...
  const themeEffectContainer = document.getElementById('themeEffectContainer'); // New container for theme-specific effects
  ...
  function startOnamFlowerShower() {
    stopAllThemeEffects(); // Clear any existing interval first
    if (flowerContainer) {
  ...
  function createPeacockFeather() {
    ...
    // Using a Unicode character that resembles a feather or just a dot for subtle effect
  ...
  function selectTheme(themeName) {
    ...
    // No specific JS effects for Ramadan, Easter, Stranger Things, GoT for now (CSS handles it)
  }
  ...
  const layoutToggleDesktop = document.getElementById('layoutToggleDesktop');
  const layoutToggleMobile = document.getElementById('layoutToggleMobile');

  /**
   * Applies the selected layout preference to the body.
   * @param {'mobile' | 'desktop'} layout - The layout to apply.
   */
  function applyLayoutPreference(layout) {
    document.body.classList.remove('layout-mobile', 'layout-desktop'); // Clear existing
    if (layout === 'mobile') {
      document.body.classList.add('layout-mobile');
    } else if (layout === 'desktop') {
      document.body.classList.add('layout-desktop');
    }
    localStorage.setItem('preferredLayout', layout); // Save preference
  }

  if (layoutToggleDesktop && layoutToggleMobile) {
    layoutToggleDesktop.addEventListener('click', () => {
      console.log("Desktop layout selected!");
      applyLayoutPreference('desktop');
    });

    layoutToggleMobile.addEventListener('click', () => {
      console.log("Mobile layout selected!");
      applyLayoutPreference('mobile');
    });
    ...
    if (savedLayout) {
      ...
      // Default to desktop layout if no preference saved
      applyLayoutPreference('desktop');
    }
  }
});
